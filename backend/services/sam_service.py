...
def _load_sam_model():
    ...
    if not SAM_CHECKPOINT.exists():
        raise FileNotFoundError(
            ...
            f"Download vit_b: https://dl.fbaipublicfiles.com/segment_anything/sam_vit_b_01ec64.pth\n"
            f"Place in: {SAM_CHECKPOINT.parent}"
        )

    global device

    # Load weights on CPU first (always safe)
    ...
    if device == "cuda":
        try:
            ...
            logger.warning(f"Failed to move SAM to CUDA: {e} — using CPU")
            device = "cpu"  # update global so future loads skip CUDA
            sam = sam.to("cpu").eval()
    else:
        sam = sam.to("cpu").eval()
